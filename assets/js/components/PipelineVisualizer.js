export default class PipelineVisualizer {
  ...
  calculateTotalHeight(pipelinesData) {
    return (
      pipelinesData.length * (this.options.pipelineHeight + this.options.pipelineMargin) +
      this.options.pipelineMargin
    );
  }
  ...
  createPipelineGroup(pipeline, index, total) {
    ...
    const yPos =
      (total - 1 - index) * (this.options.pipelineHeight + this.options.pipelineMargin) +
      this.options.pipelineMargin;
    ...
    const [startEmptyPipe, startArrow] = this.createEmptyPipeElement(
    ...
    const startLabel = this.createCityLabel(
      startCity,
    ...
    const endLabel = this.createCityLabel(
      endCity,
  ...
  createPipeElement(pipeline_title, pipe, yPos, index, pipeWidth) {
    ...
    if (Array.isArray(pipe.departments)) {
      if (pipe.departments.length === 1) {
        ...
        departmentInfo = pipe.departments
          .map((dep) => {
  ...
  createNodeElement(pipeline_title, node, yPos, pipeline, pipeWidth) {
    ...
    if (node.node_type === 'bridge') {
      const pipe = pipes.find(
        (p) => node.location_point >= p.start_point && node.location_point <= p.end_point
      );
      if (!pipe) return null;
      const pipeIndex = pipes.indexOf(pipe);
      const localPercent =
        (node.location_point - pipe.start_point) / (pipe.end_point - pipe.start_point);
      ...
      const pipeStartX =
        containerWidth - this.options.pipelinePadding - (pipeIndex + 2) * pipeWidth;
  ...
  createValveElement(node, x, y, size) {
    ...
    group.setAttribute(
      'transform',
      `translate(${x - 10 * scale}, ${y - 10 * scale}) scale(${scale})`
    );
    const getValveColor = () => {
