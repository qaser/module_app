...
function imagesCard(item) {
  const image = new Image(item, '.image-template', handleImageClick, handleImageDeleteClick);
  const instance = image.renderImage();
  return instance;
}

function filesCard(item) {
  const file = new FileManager(item, '.file-template', handleFileDeleteClick);
...
function handleFileDeleteClick(evt, fileId) {
  ...
  // renderLoading()
  api.deleteValveFile(id).then((res) => {
    if (res.status == 204) {
...
function handleImageDeleteClick(evt, imageId) {
  ...
  // renderLoading()
  api.deleteValveImage(id).then((res) => {
    if (res.status == 204) {
...
function uploadImage(file) {
  ...
  formData.append('valve_id', valveId);
  // renderLoading()
  api.addValveImage(formData, valveId).then((data) => {
...
Promise.all([initUser(), api.getValveItem(valveId)])
  .then(([userData, valve]) => {
    valveInstance.renderItem(valve);
