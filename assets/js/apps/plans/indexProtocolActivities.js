...
function submitFormNewActivity(data) {
    ...
    // Отправляем запрос
    api.createProtocolActivity(formData)
        .then((activity) => {
...
function submitFormMarkActivity(data) {
    ...
    api.markProtocolActivity(activityId, { status, actual_completion_date, comment })
        .then(() => {
...
function submitConfirmArchive() {
    ...
    api.changeProtocolActivity(activityId)
        .then(() => {
