...
// const protocolId = document.querySelector('.main__title').id;
const actionButtons = document.querySelectorAll('.activity-card__button[data-action]');
// const addNewActivityBtn = document.querySelector('#addNewActivityBtn');
// const deleteProtocolBtn = document.querySelector('#deleteActivityBtn');
...
function submitFormNewActivity(data) {
  ...
  const selectedDepartmentIds = Array.from(
    depsContainer.querySelectorAll('.form-popup__tag-btn[data-selected="true"]')
  ...
  const deadline_date =
    data.deadline_type === 'date' && data.deadline_date ? data.deadline_date : null;
  ...
  // Отправляем запрос
  api
    .createProtocolActivity(formData)
    .then((activity) => {
...
function submitFormMarkActivity(data) {
  ...
  api
    .markProtocolActivity(activityId, { status, actual_completion_date, comment })
    .then(() => {
...
function submitConfirmDelete() {
  api
    ...
    .then(() => {
      ...
      location.href = '/plans/protocols/';
    })
    .catch((err) => {
      console.error('Ошибка при удалении протокола:', err);
    });
}
...
function submitConfirmArchive() {
  ...
  api
    .changeProtocolActivity(activityId)
    .then(() => {
