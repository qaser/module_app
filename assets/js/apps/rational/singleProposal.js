...
function submitFormEditProposal(data) {
  ...
  // renderLoading()
  api
    .changeProposal(formData, proposalId)
    .then((proposal) => {
...
function handleFileDeleteClick(evt, fileId) {
  ...
  // renderLoading()
  api.deleteProposalFile(id).then((res) => {
    if (res.status == 204) {
...
Promise.all([initUser(), api.getProposalItem(proposalId)])
